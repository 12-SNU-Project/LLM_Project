...
class AuditReportRAG:
    ...
    def __init__(
        ...
        self.sections = self.chroma_client.get_or_create_collection("audit_sections")
    ...
    def query_vector(self, query: str, top_k: int = 5) -> Dict:
        """벡터DB에서 관련 섹션 검색"""
        # 공통 연도 추출 함수 사용
        year = self._extract_year(query)
        where_filter = {"year": year} if year else None
        
        # 질문 정규화 (연도 표현 통일, 공백 제거)
        ...
        for i in range(len(results["ids"][0])):
            sections.append({
                ...
                "section_num": results["metadatas"][0][i]["section_num"],
...
**답변:**"""
    ...
    def ask(self, question: str, use_llm: bool = True) -> str:
        ...
        if data_count == 0:
            ...
            if result["source"] == "sqlite" and result["query_type"] == "balance_sheet":
                target = self._extract_account_keyword(question, year)
                print(f"[디버그] 추출된 연도: {year}")
                print(f"[디버그] 추출된 계정과목: {target}")
                
                # 해당 연도 행 수 확인
                cursor = self.sqlite_conn.cursor()
                if year:
                    cursor.execute("SELECT COUNT(*) FROM balance_sheet WHERE year = ?", (year,))
                else:
                    cursor.execute("SELECT COUNT(*) FROM balance_sheet")
                count = cursor.fetchone()[0]
                print(f"[디버그] DB 내 해당 연도 행 수: {count}개")
            
            return "관련 데이터를 찾지 못했습니다. 질문의 연도나 계정과목 표현을 확인해주세요."
