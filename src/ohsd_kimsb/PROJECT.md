...
6. 근거 부족 시 `tables_registry`와 `table_rows`에서 표 전체 맥락과 행 구조 추가 조회
7. 최종 답변 생성
...
### `metric_facts`
표의 숫자값을 빠르게 찾기 위한 projection/fact 테이블이다.
...
### `table_rows`
표의 행 계층과 회사 분류를 보존하는 row registry다.
...
### Canonical Views
entity/table 질의와 이후 constrained text-to-sql을 위해 아래 view를 함께 노출한다.

- `related_party_balance_view`
- `related_party_transaction_view`
- `subsidiary_status_view`
- `subsidiary_summary_view`
...
- `sql_templates.py`: 템플릿 기반 SQL 생성/실행 + 표/행 컨텍스트 조회
...
9. `service/hybrid_qa.py`에서 부족 근거 축에 맞춰 `tables_registry`와 `table_rows` 기반 표/행 컨텍스트 추가 조회
10. `support/answering/composer.py`

## 표 파싱 전략

### 1. 구조를 먼저 보존한다
HTML은 먼저 markdown으로 바꾸지 않고, 아래 block으로 분해한다.
...
이 단계의 목적은 "표와 본문이 원래 문서 안에서 어떤 순서와 문맥을 가졌는지"를 먼저 보존하는 것이다.

### 2. 표는 숫자 집합이 아니라 구조 객체로 정규화한다
`table_processor.py`는 각 `<table>`을 아래 순서로 정규화한다.

1. `rowspan/colspan`을 해체해 logical grid를 만든다.
2. `(단위 : 백만원)` 같은 단위 행을 추출하고, `계속;` 같은 노이즈 행은 제거한다.
3. 열 header path를 복원해 각 숫자가 어떤 열 의미를 갖는지 붙인다.
4. section header와 들여쓰기를 이용해 row hierarchy를 복원한다.
5. 표 내부 footnote marker와 표 하단 footnote text를 추출한다.
6. 표 전체를 markdown으로 저장한다.
7. 숫자 셀은 `metric_facts`로, 행 구조는 `table_rows`로, 표 전체는 `tables_registry`로 나눈다.
8. 마지막에 표 의미를 `semantic_table_type`으로 분류한다.

즉 하나의 표는 세 개의 다른 관점으로 저장된다.

- `metric_facts`: 빠른 숫자 조회용 projection
- `table_rows`: 행 계층과 회사 분류를 보존하는 구조 레지스트리
- `tables_registry`: 표 제목, 단위, 각주, markdown을 보존하는 표 객체

### 3. 본문은 표와 연결된 설명 단위로 저장한다
본문은 section-aware chunking으로 자른다.

1. 섹션 제목을 먼저 인식한다.
2. 섹션 내부 문단을 모은다.
3. 길이 제한을 넘으면 문단 경계로 분리한다.
4. 각 chunk에 `section_type`, `section_title`, `page`, `near_table_id`를 저장한다.

그래서 이후 설명 검색은 단순 문장 검색이 아니라, "어느 표 근처의 어떤 설명인지"까지 함께 추적할 수 있다.

## 사용자 질문 해석 전략

### 1. 언어적 중의성을 줄이기 위해 앞단에서 LLM으로 먼저 분류한다
사용자 질문은 짧거나 모호할 수 있고, 같은 표현도 숫자 조회, 설명 조회, 표 구조 조회로 다르게 해석될 수 있다.
이 문제를 줄이기 위해 앞단에서 `LangChainQueryInterpreter`가 LLM을 사용해 질문을 먼저 구조화한다.

LLM이 바로 하는 일은 아래 두 가지다.

- 질문의 큰 intent를 정한다.
- 연도, 회사명, 행/열 anchor, section 단서 같은 보조 slot을 JSON으로 뽑는다.

### 2. 질의 해석은 자유 생성이 아니라 제한된 schema 안에서만 허용한다
`build_llm_instruction()`은 LLM에게 아래 규칙을 강하게 준다.

- intent는 고정된 집합 중 하나만 선택
- SQL을 직접 만들지 말 것
- `metric_candidates`, `row_label_filters`, `column_terms`, `table_title_terms`, `year`, `period`, `entity_scope`, `section_candidates` 같은 slot만 채울 것
- 질문이 너무 추상적이면 `clarification_needed=true` 로 돌릴 것
- 출력은 JSON만 할 것

즉 해석 LLM은 "질문 의미를 구조화하는 분류기"이지, 직접 검색을 수행하는 agent가 아니다.

### 3. LLM 해석만 그대로 믿지 않고 rule-based fallback과 병합한다
질의 해석은 두 경로를 함께 쓴다.

- LLM 해석: 언어적 중의성과 다양한 표현을 흡수
- rule-based 해석: metric alias, 연도, 행/열 키워드, 회사명 anchor를 안정적으로 복원

최종 해석은 둘을 병합한 뒤 `QueryRoutingPolicy`가 다시 고정한다.
이 정책 계층은 다음을 담당한다.

- `need_sql / need_vdb / clarification_needed` 최종 확정
- 연도 token이 row/column anchor로 잘못 들어가는 문제 정리
- broad qualitative question을 무조건 VDB로 보내지 않고 clarification으로 되돌리기
- intent별 minimum anchor 검사

즉 구조는 `LLM -> fallback rules -> routing policy` 순서다.

### 4. 기존 intent를 늘리기보다 기존 intent 안에서 anchor를 정교화한다
현재 방향은 새 intent를 계속 추가하는 방식이 아니다.
기존 intent 안에서 질문 anchor를 더 정확히 잡도록 보강했다.

- 회사명 + 열 의미(`채권`, `채무`, `매출`, `매입`, `지분율`)가 같이 보이면 `table_cell_lookup` 쪽으로 보낸다.
- 회사 분류 질문(`종속기업/관계기업/공동기업`)은 별도 intent를 만들지 않고 `company_kind`가 적재된 SQL evidence로 답한다.
- entity-specific 셀 조회는 의미에 따라 `related_party_balance_table`, `related_party_transaction_table`, `subsidiary_status_table`을 우선 정렬한다.

이 접근을 택한 이유는 intent를 무한정 늘리면 기존 질의 안정성이 흔들리기 쉽기 때문이다.

## 온라인 검색 및 보강 전략

### 1. 1차 검색은 질문 유형에 맞는 최소 경로로 시작한다
질문 해석 결과로 `need_sql / need_vdb / clarification_needed`를 확정한 뒤 1차 검색을 수행한다.
...
여기서 SQL은 빠른 값 탐색, VDB는 설명 문맥 탐색 역할을 맡는다.

### 2. 부족한 근거만 추가하는 evidence-gap 기반 보강을 사용한다
...
그 다음 SQL 결과의 `table_id`와 vector hit의 `near_table_id`를 함께 후보로 모아 `tables_registry`와 `table_rows`를 조회한다.
즉 특정 질의 타입별 하드코딩보다, "지금 무엇이 부족한가"를 기준으로 표 보강 여부를 결정한다.

이 단계에서 주로 가져오는 것은 아래다.
...
### 3. 표 전체를 그대로 넣지 않고 질문에 맞게 축약한다
answer 단계에는 표 원문 전체를 그대로 넣지 않는다.
질의와 SQL anchor에 맞춰 아래처럼 잘라낸 근거를 우선 사용한다.

- `focused_table_markdown`
- `focused_footnotes`
- `focused_rows`

즉 "표가 필요하면 표 전체를 넣는 방식"이 아니라 "관련 행/열/각주/행 구조만 축약해서 넣는 방식"이다.

## LLM 조교(Instruction) 전략

### 1. 질의 해석 LLM은 분류기로만 사용한다
질의 해석 단계의 system prompt는 LLM이 아래 역할만 하도록 제한한다.

- 질문을 고정된 intent 집합 중 하나로 분류
- 정해진 slot schema를 채움
- SQL 생성 금지
- JSON 외 출력 금지

즉 이 단계의 LLM은 "자유로운 agent"가 아니라 "구조화된 질의 해석기"다.

### 2. 최종 답변 LLM은 evidence 기반 생성기로만 사용한다
`LangChainAnswerComposer`의 system prompt는 아래 규칙을 준다.

- 반드시 제공된 evidence만 사용
- 근거가 부족하면 부족하다고 명시
- 마지막에 출처를 간단히 명시

user prompt에는 아래 근거가 함께 들어간다.
...
- `SQL evidence`
- `Text evidence`
- `Table context evidence`

즉 최종 답변 LLM도 "검색을 새로 하는 모델"이 아니라 "정리된 근거를 읽고 답을 렌더링하는 모델"이다.

### 3. LLM 출력이 흔들려도 deterministic layer가 다시 안정화한다
현재 구조에서 LLM은 강력하지만 단독 의사결정자가 아니다.
중간과 마지막 단계에 항상 deterministic layer가 들어간다.

- 질의 해석 단계: `QueryRoutingPolicy`
- 검색 단계: `SQLTemplateEngine`, `EvidenceExpansionPlanner`
- 답변 단계: evidence bundle 기반 prompt 조립과 fallback formatter

이렇게 해서 LLM의 언어 이해력은 활용하되, 검색 경로와 답변 근거는 시스템이 다시 고정한다.
